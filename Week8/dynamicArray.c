...
void addHeap(DynArr *heap, TYPE val)
{
   ...
   while(compare(getDynArr(heap,parent), getDynArr(heap,i)) == 1){
      swapDynArr(heap, parent, i);
      i = parent;
      parent = (i - 1) / 2;
   }
   return;
...
{
    assert(heap != 0);
    int notLeaves = sizeDynArr(heap) / 2;

    for(int i = notLeaves; i >= 0; i--){
...
void sortHeap(DynArr *heap)
{
   ...
   int j = sizeDynArr(heap) -1;

   _buildHeap(heap); 
   while(j >= 0){
      swapDynArr(heap,j,0);
      _adjustHeap(heap,j,0);
      j--;
   }
