...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	struct Vertex* curVertex = source;
        curVertex->isVisited = 1;

	struct cirListDeque* neighbors = (struct cirListDeque*)malloc(sizeof(struct cirListDeque));
	initCirListDeque(neighbors);
	for(int i = 0; i < curVertex->numNeighbors; i++){
		addFrontCirListDeque(neighbors, curVertex->neighbors[i]);
	}

	while(!isEmptyCirListDeque(neighbors)){

		curVertex = frontCirListDeque(neighbors);
		removeFrontCirListDeque(neighbors);
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	while(!isEmptyCirListDeque(neighbors)){
		curVertex = backCirListDeque(neighbors);
		removeBackCirListDeque(neighbors);

		if(curVertex == destination){
			return 1;
		}

		if(curVertex->isVisited == 1){
			continue;
		} else {
			curVertex->isVisited = 1;
		}
